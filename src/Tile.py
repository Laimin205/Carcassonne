...
Модуль для работы с даннми тайла

Содержит класс Tile, который отвечает за:
- создание тайла из спрайт-листа
...
Зависимости:
- settings.py: содержит константу TILE_SIZE
"""
import pygame as pg
...
class Tile:
    ...
    - _status (str): текущее состояние тайла
        * bag - "в мешке", позиция дефолтная, не отображается
        * hold - "примеряется" на поле, позиция меняется, полупрозрачно - TODO
        * placed - размещен на поле
    """
    ...
    def draw(self, window):
        """
        Отображает тайл в окне на основе текущего состояния
    ...
    def draw_pos(self, window, x = 0, y = 0, scale = 1):
        ...
        Args:
        - window (Graphics): класс окна, предоставляющий необходимые для отображения данные
    ...
    def rotate(self):
        """
        Поворачивает тайл на 90 градусов по часовой стрелке
        """
